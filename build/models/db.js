...
export default class db {
    ...
    static async getUser(userId) {
        const user = await this.client.db("abrnoc").collection("customer").findOne({ _id: mongodb.ObjectId.createFromHexString(userId) }, { projection: { password: 0 } });
        if (user) {
            return user;
        }
        else {
    ...
    static async getInvoices(userId) {
        ...
        const result = await this.client.db("abrnoc").collection("invoice").find({
            userId: mongodb.ObjectId.createFromHexString(userId),
            endTime: { $exists: true }
        }).toArray();
        return result;
    ...
    static async deleteSub(subId) {
        ...
        const result = await this.client.db("abrnoc").collection("subs").deleteOne({ _id: mongodb.ObjectId.createFromHexString(subId) });
        return result.acknowledged;
    }
